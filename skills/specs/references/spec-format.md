...
- A plan says: "The system models entities with three fields."
- A spec says: "An Entity with an empty description cannot exist. Whitespace-only strings are treated as empty. Construction raises a validation error."
...
When two domains communicate (e.g., service-a ↔ service-b, service-b ↔ service-c), the message contract between them is a **protocol spec**. Protocol specs are bilateral contracts — both sides depend on them, neither side owns them unilaterally.
...
protocols/
├── <protocol-name>/
│   └── specs/         # service-a ↔ service-b message contract
└── <protocol-name>/
    └── specs/         # service-b ↔ service-c message contract
```
